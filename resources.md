...
Each function is described below, showing the default and
allowed values that may be in
the `Result` term. The default will be
used if a resource does not export the function.  If a
function has an "X" in the "Halt" column, it also has the
option of returning either of the two following special values
for `Result`:
...
Any function which has no description is optional and the
effect of its return value should be evident from examining
the [diagram](diagram.html).
...
| `delete_resource`| `false` | X | `true | false` |  This is called when a DELETE request should be enacted, and should return true if the deletion succeeded. |
| `delete_completed`| `true` | X | `true | false` |  This is only called after a successful delete_resource call, and should return false if the deletion was accepted but cannot yet be guaranteed to have finished. |
| `post_is_create`| `false` | | `true | false` |  If POST requests should be treated as a request to put content into a (potentially new) resource as opposed to being a generic submission for processing, then this function should return true. If it does return true, then create_path will be called and the rest of the request will be treated much like a PUT to the Path entry returned by that call. |
